...
/** GET EMI invoice (PDF or meta) — {@code /api/emi/pgmid/{pgmid}/getInvoice/{merchantTransId}}. */
public class DealsEmiGetInvoice extends BaseApi {

    private static final String PATH_TEMPLATE = "/api/emi/pgmid/%s/getInvoice/%s";
    ...
    private DealsEmiGetInvoice(Builder b) {
        setMethod(MethodType.GET);
        String base = b.baseUri != null ? b.baseUri : LocalConfig.PLE_DEALS_BASE_URL;
        getRequestSpecBuilder().setBaseUri(base);
        getRequestSpecBuilder().setBasePath(String.format(PATH_TEMPLATE, b.pgmid, b.merchantTransId));
        ...
        getRequestSpecBuilder().addQueryParam("run", b.run);
    ...
    public static final class Builder {
        ...
        public Builder merchantTransId(String merchantTransId) {
        ...
        public DealsEmiGetInvoice build() {
            if (pgmid == null || merchantTransId == null) {
                throw new IllegalStateException("pgmid and merchantTransId are required");
            }
            if (requestId == null || requestId.isEmpty()) {
                requestId = "PAYTM_" + Long.toHexString(System.nanoTime()).toUpperCase();
            }
            return new DealsEmiGetInvoice(this);
        }
    }
}
