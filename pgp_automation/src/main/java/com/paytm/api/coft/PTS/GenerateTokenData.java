 ...
 * @Version 1.0
 */
public class GenerateTokenData extends BaseApi {
    ...
    public GenerateTokenData(String mid) {
        setMethod(MethodType.POST);
        getRequestSpecBuilder().setContentType(ContentType.JSON);
        getRequestSpecBuilder().setAccept(ContentType.JSON);
        getRequestSpecBuilder().addHeader("X-Amzn-Trace-Id", PGPHelpers.TraceIdGenerator());
        getRequestSpecBuilder().setBaseUri(LocalConfig.COFT_CENTER);
        getRequestSpecBuilder().setBasePath(Constants.coftCenter.GENERATE_TOKEN_DATA);
    ...
    public GenerateTokenData buildRequest(String paymentType, String tokenIndexNumber, String custId, String userId) {
    ...
    public GenerateTokenData generateChecksum(String key) {
        Gson gson = new Gson();
