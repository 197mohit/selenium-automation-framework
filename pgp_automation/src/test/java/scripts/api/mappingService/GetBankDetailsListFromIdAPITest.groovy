...
import static com.paytm.appconstants.Constants.MappingService.GET_BANK_DETAILS_LIST_FROM_ID
import static io.restassured.RestAssured.given
import static io.restassured.module.jsv.JsonSchemaValidator.matchesJsonSchemaInClasspath
import static org.hamcrest.Matchers.*

class GetBankDetailsListFromIdAPITest extends TestSetUp {

    private static final String URL_PATH_PARAM_BANK_IDS = 'bankIds'
    ...
    private final RequestSpecBuilder reqBldr() {
        new RequestSpecBuilder()
                ...
                .setBaseUri(PGP_HOST)
                .setBasePath(GET_BANK_DETAILS_LIST_FROM_ID)
    }
        ...
        Response filter(FilterableRequestSpecification requestSpec, FilterableResponseSpecification responseSpec, FilterContext ctx) {
            responseSpec.spec(
                    new ResponseSpecBuilder()
                            .expectBody(matchesJsonSchemaInClasspath('json-schemas/mapping-service/get-bank-details-list-from-id-schema.json'))
                            .build()
            )
    ...
    void testSuccess() {
        ...
        def row = rows[0]
        req().pathParam(URL_PATH_PARAM_BANK_IDS, row[Column.BANK_ID]).get().then()
                .root('response')
