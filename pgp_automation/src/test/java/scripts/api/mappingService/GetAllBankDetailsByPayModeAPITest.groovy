...
import static com.paytm.appconstants.Constants.MappingService.GET_ALL_BANK_DETAILS_BY_PAY_MODE
import static io.restassured.RestAssured.given
import static io.restassured.module.jsv.JsonSchemaValidator.matchesJsonSchemaInClasspath
import static org.hamcrest.Matchers.equalTo
import static org.hamcrest.Matchers.hasSize

class GetAllBankDetailsByPayModeAPITest extends TestSetUp {

    private static final String URL_PATH_PARAM_PAY_MODE = 'payMode'
    ...
    private final RequestSpecBuilder reqBldr() {
        new RequestSpecBuilder()
                ...
                .setBaseUri(PGP_HOST)
                .setBasePath(GET_ALL_BANK_DETAILS_BY_PAY_MODE)
    }
        ...
        Response filter(FilterableRequestSpecification requestSpec, FilterableResponseSpecification responseSpec, FilterContext ctx) {
            responseSpec.spec(
                    new ResponseSpecBuilder()
                            .expectBody(matchesJsonSchemaInClasspath('json-schemas/mapping-service/get-all-bank-details-by-pay-mode-schema.json'))
                            .build()
            )
    ...
    @Test
    void testSuccess() {
        final String PAY_MODE = 'NB'
        String query = "SELECT * FROM BANK_MASTER WHERE STATUS = 1 AND PAY_MODE = '$PAY_MODE' AND BANK_NAME IS NOT NULL AND BANK_CODE IS NOT NULL ORDER BY BANK_NAME ASC;"
        ...
        req()
                .pathParam(URL_PATH_PARAM_PAY_MODE, PAY_MODE)
                .get().then()
                .root('response')
                .body(
                ...
                .body(
                        ...
                        'extIfscCode', equalTo((rows.collect { it[Column.EXT_IFSC_CODE] })),
                )
    }
}