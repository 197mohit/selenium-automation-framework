...
import static com.paytm.appconstants.Constants.MappingService.GET_LOOK_UP_DATA_FROM_CATEGORY_AND_SUB_CATEGORY
import static io.restassured.RestAssured.given
import static io.restassured.module.jsv.JsonSchemaValidator.matchesJsonSchemaInClasspath
import static org.hamcrest.Matchers.equalTo

class GetLookUpDataFromCategoryAndSubCategoryAPITest extends TestSetUp {
    ...
    private final RequestSpecBuilder reqBldr() {
        new RequestSpecBuilder()
                ...
                .setBaseUri(PGP_HOST)
                .setBasePath(GET_LOOK_UP_DATA_FROM_CATEGORY_AND_SUB_CATEGORY)
    }
        ...
        Response filter(FilterableRequestSpecification requestSpec, FilterableResponseSpecification responseSpec, FilterContext ctx) {
            responseSpec.spec(
                    new ResponseSpecBuilder()
                            .expectBody(matchesJsonSchemaInClasspath('json-schemas/mapping-service/get-look-up-data-from-category-and-sub-category-schema.json'))
                            .build()
            )
    ...
    @Test
    void testSuccess() {
        String query = "SELECT * FROM PAYTMPGDB.LOOKUP_DATA WHERE STATUS=9376503 AND CATEGORY IS NOT NULL AND SUB_CATEGORY IS NOT NULL AND CATEGORY != 'PREFERENCES' LIMIT 1;"
        ...
        query = "SELECT * FROM PAYTMPGDB.LOOKUP_DATA WHERE STATUS=9376503 AND CATEGORY='${CATEGORY}' AND SUB_CATEGORY='${SUB_CATEGORY}';"
