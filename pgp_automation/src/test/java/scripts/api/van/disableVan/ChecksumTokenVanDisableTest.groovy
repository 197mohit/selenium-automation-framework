...
class ChecksumTokenVanDisableTest  extends TestSetUp implements VanDisableTest , ChecksumAuthenticationTest{
...
//    @Merchant(edit = false, value = { it.preferences.bankTransfer.enabled == true && it.acquirings.any { it.payMode == 'bank-transfer' && it.enabled == true } })
    ...
    @Test
    @Merchants([
            @Merchant({ it.preferences.bankTransfer.enabled == true }),
            @Merchant({ it.preferences.bankTransfer.enabled == false })
    ])
    void "test when head token equals checksum generated using different merchant's key"() {
        ...
        req().body(root).post().then().
                body("body.resultInfo.resultStatus", equalTo("F")).
                body("body.resultInfo.resultCode", equalTo("2002")).
                body("body.resultInfo.resultMsg", startsWith("CheckSum Validation Failure"))
