...
class TxnTokenApplyPromoV2Test extends TestSetUp implements ApplyPromoTest, TxnTokenAuthenticationTest, HeadTest, TxnTokenApplyPromoTest {
    ...
    private final static PROMO_CODE = "PROMOABHI";
    ...
    private final static class ResultInfo {
        ...
        static ResponseSpecification TNCURL_FF4JFLAG_ENABLED = new ResponseSpecBuilder()
                .rootPath('body.resultInfo')
                .expectBody('resultStatus', equalTo('S'))
                .expectBody('resultCode', equalTo('00000000'))
                .expectBody('resultMsg', equalTo("Success"))
                .rootPath('body.paymentOffer')
                .expectBody('tncUrl', notNullValue())
                .build()
        static ResponseSpecification TNCURL_FF4JFLAG_DISABLED = new ResponseSpecBuilder()
                .rootPath('body.resultInfo')
                .expectBody('resultStatus', equalTo('S'))
                .expectBody('resultCode', equalTo('00000000'))
                .expectBody('resultMsg', equalTo("Success"))
                .rootPath("body")
                .expectBody("paymentOffer", not(hasKey("tncUrl")))
                .build()
        static ResponseSpecification CASHBACK_AMOUNT_IS_NOT_NULL = new ResponseSpecBuilder()
                .rootPath('body.resultInfo')
                .expectBody('resultStatus', equalTo('S'))
                .expectBody('resultCode', equalTo('00000000'))
                .expectBody('resultMsg', equalTo("Success"))
                .rootPath('body.paymentOffer.offerBreakup')
                .expectBody("cashbackAmount", notNullValue())
                .expectBody('promotext', notNullValue())
                .build()
        static ResponseSpecification INSTANT_DISCOUNT_IS_NOT_NULL = new ResponseSpecBuilder()
                .rootPath('body.resultInfo')
                .expectBody('resultStatus', equalTo('S'))
                .expectBody('resultCode', equalTo('00000000'))
                .expectBody('resultMsg', equalTo("Success"))
                .rootPath('body.paymentOffer.offerBreakup')
                .expectBody("instantDiscount", notNullValue())
                .expectBody("promotext", notNullValue())
                .build()
    }
    ...
    void 'test when body promocode equals expired promocode'() {
       ...
       Response r= req().body(root).post().then().extract().response();
                //.spec(ResultInfo.NOT_VALID_ANYMORE)
...
//    @Test(enabled = false)
    ...
    @Merchant(edit = true, value = { it.id == Constants.MerchantType.BRAND_BO_DISC_BAJAJ.getId() })
    @Test
    void "test tncUrl when FF4J applyPromoSendResponseTncUrl flag is enabled on mid"() {
        def root = root()
        if (!m().promos.add(new Promo())) throw new SkipException(PROMOS_NOT_ADDED)
        root.body.mid = Constants.MerchantType.BRAND_BO_DISC_BAJAJ.getId()
        root.body.promocode = PROMO_CODE
        req().body(root).post().then()
                .spec(ResultInfo.TNCURL_FF4JFLAG_ENABLED)
    }
    ...
    @Merchant(edit = true, value = { it.id == Constants.MerchantType.BRAND_BO_DISC_BAJAJ_HDFC.getId() })
    @Test
    void "test tncUrl when FF4J applyPromoSendResponseTncUrl flag is disabled on mid"() {
        def root = root()
        if (!m().promos.add(new Promo())) throw new SkipException(PROMOS_NOT_ADDED)
        root.body.mid = Constants.MerchantType.BRAND_BO_DISC_BAJAJ_HDFC.getId()
        root.body.promocode = PROMO_CODE
        req().body(root).post().then()
                .spec(ResultInfo.TNCURL_FF4JFLAG_DISABLED)
    }
    ...
    @Merchant(edit = true , value = { it.id == Constants.MerchantType.BRAND_BO_DISC_BAJAJ_HDFC.getId() })
    @Test
    void "cashbackAmount should not be null if promo is applied"() {
        def root = root()
        if (!m().promos.add(new Promo())) throw new SkipException(PROMOS_NOT_ADDED)
        root.body.mid = Constants.MerchantType.BRAND_BO_DISC_BAJAJ_HDFC.getId()
        root.body.promocode = PROMO_CODE
        req().body(root).post().then()
                .spec(ResultInfo.CASHBACK_AMOUNT_IS_NOT_NULL)
    }
    ...
    @Merchant(edit = true , value = { it.id == Constants.MerchantType.BRAND_BO_DISC_BAJAJ_HDFC.getId() })
    @Test
    void "instantDiscount should not be null if promo is applied"() {
        def root = root()
        if (!m().promos.add(new Promo())) throw new SkipException(PROMOS_NOT_ADDED)
        root.body.mid = Constants.MerchantType.BRAND_BO_DISC_BAJAJ_HDFC.getId()
        root.body.promocode = 'discout'
        req().body(root).post().then()
                .spec(ResultInfo.INSTANT_DISCOUNT_IS_NOT_NULL)
    }
}
