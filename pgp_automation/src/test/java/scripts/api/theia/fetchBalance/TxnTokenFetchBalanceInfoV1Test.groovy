...
class TxnTokenFetchBalanceInfoV1Test extends TestSetUp implements FetchBalanceInfoTest, TxnTokenAuthenticationTest, HeadTest {
    ...
    void 'test when orderId is not provided in query params'() {
        def root = root()
        Response r=given(reqBldr().removeQueryParam('orderId').build())
                .body(root).post().then().extract().response();
                //.spec(resSpec)
               // .spec(results.orderIdMandatoryInQueryParams as ResponseSpecification)
        JsonPath res=r.jsonPath();
        Assert.assertEquals(res.getString("body.resultInfo.resultMsg"),"OrderId in the query param doesn't match with the OrderId send in the request");
    }
    ...
    @Override
    @Merchant({ it.id=="SjPzYc26628147795378" })
