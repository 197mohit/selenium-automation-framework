...
@Owner('Deepak')
class GetCardFlagOffTest extends GetCardTest {

    @BeforeClass
    void beforeClass() {
        String queryForExp = "UPDATE FF4J_FEATURES SET ENABLE = 0 WHERE FEAT_UID = '$SC_FETCH_FROM_PLATFORM_FOR_MID';";
    ...
    void 'test able to fetch saved card(s) when REQUEST_TYPE is not provided'() {
        def root = root()
        assert m().users[0].savedCards.add(cards.find())
    ...
    void 'test unable to fetch saved card(s) when REQUEST_TYPE equals random value'() {
        ...
        req().body(root).post().then()
                .statusCode(500)
                .spec(errorSchema)
                .body('response', equalTo(SYSTEM_ERROR))
    }
    ...
    @Override
    @Merchant({ it.acquirings.any { it.bank == 'bajajfn' } && it.preferences.saveCard.enabled == true })
    @AUser(edit = true)
    @Test
    ...
    @Override
    @Merchant({ it.acquirings.any { it.payMode == 'dc' && !(it.bank in ['bajajfn', 'amex']) } })
    @AUser(edit = true)
    @Test
    void 'test able to fetch saved card when debit card is saved on userId'() {
        def root = root()
        user().savedCards.clear()
        assert user().savedCards.add(cards.find { it.type == 'debit' && !(it.scheme in ['bajajfn', 'amex']) })
        root.CUSTID = null
        root.SSO_TOKEN = user().tokens['sso'].id
        req().body(root).post().then()
                .spec(successSchema)
    ...
    void 'test able to fetch saved card(s) when cards are saved on userId and custId and mId'() {
        ...
        if (!m().users[0].savedCards.add(card1)) throw new SkipException('unable to save card on custId and mId')
        if (!user().savedCards.add(card2)) throw new SkipException('unable to save card on userId')
        root.CUSTID = m().users[0].id
        root.SSO_TOKEN = user().tokens['sso'].id
        req().body(root).post().then()
                .spec(successSchema)
    ...
    void 'test able to fetch saved card(s) when same card is saved on both custId & mId and userId'() {
        ...
        user().savedCards.clear()
        cards.find().with {
            assert m().users[0].savedCards.add(it)
            assert user().savedCards.add(it)
        }
        root.CUSTID = m().users[0].id
        root.SSO_TOKEN = user().tokens['sso'].id
        req().body(root).post().then()
