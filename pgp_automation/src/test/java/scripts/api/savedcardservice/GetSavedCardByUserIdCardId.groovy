...
@Owner('Deepak')
class GetSavedCardByUserIdCardId extends TestSetUp {
    ...
    final def req = { userId, cardId ->
        given(
                new RequestSpecBuilder()
                        ...
                        .setBaseUri(PGP_HOST)
                        .setBasePath("/savedcardservice/savedCardService/v1/get/savedcard/userId/cardId/${userId}/${cardId}")
                        .build()
        )
    }
    ...
    void testSuccess() {
        ...
        req(user().id, user().savedCards.find().id).get().then()
                .statusCode(200)
