...
class GetSavedCardByCustIdMIdChecksumFlagOffTest extends GetSavedCardByCustIdMIdChecksumTest {
    ...
    void "test able to fetch saved card when credit card is saved on custId and mId"() {
        m().users[0].savedCards.clear()
        assert m().users[0].savedCards.add(cards.find { it.type == 'credit' && !(it.scheme in ['bajajfn', 'amex']) })
        req().get().then()
                .spec(successSchema)
    }
    ...
    void "test able to fetch saved card when debit card is saved on custId and mId"() {
        m().users[0].savedCards.clear()
        assert m().users[0].savedCards.add(cards.find { it.type == 'debit' && !(it.scheme in ['bajajfn', 'amex']) })
        req().get().then()
                .spec(successSchema)
    }
    ...
    void 'test unable to fetch saved card(s) when checkSum equals random value'() {
        m().users[0].savedCards.clear()
        if (!m().users[0].savedCards.addAll(cards.find())) throw new SkipException('unable to save card')
