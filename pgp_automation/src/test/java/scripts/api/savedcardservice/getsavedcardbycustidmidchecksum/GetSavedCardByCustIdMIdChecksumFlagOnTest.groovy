...
class GetSavedCardByCustIdMIdChecksumFlagOnTest extends GetSavedCardByCustIdMIdChecksumTest {

    @BeforeClass
    void beforeClass() {
        String queryForExp = "UPDATE FF4J_FEATURES SET EXPRESSION = 'grantedCustIds=ALL&grantedMids=ALL', ENABLE = 1 WHERE FEAT_UID = '$SC_FETCH_FROM_PLATFORM_FOR_MID_CUSTID';";
    ...
    @Override
    @Merchant(edit = true, value = { it.acquirings.any { it.payMode == 'dc' && !(it.bank in ['bajajfn', 'amex']) } && it.acquirings.any { it.payMode == 'cc' && !(it.bank in ['bajajfn', 'amex']) } && it.preferences.saveCard.enabled == true })
    @Test
    ...
    @Override
    @Merchant(edit = true, value = { it.acquirings.any { it.bank == 'amex' } && it.preferences.saveCard.enabled == true })
    @Test
    ...
    @Override
    @Merchant(edit = true, value = { it.acquirings.any { it.bank == 'bajajfn' } && it.preferences.saveCard.enabled == true })
    @Test
...
//TODO need to write code to update expiry of card on alipay
    ...
    @Override
    @Merchant(edit = true, value = {  it.acquirings.any { it.payMode == 'cc' && !(it.bank in ['bajajfn', 'amex']) } && it.preferences.saveCard.enabled == true })
    @Test
    void "test able to fetch saved card when credit card is saved on custId and mId"() {
        (m() as AlipayMerchant).users[0].savedCards.clear()
        assert (m() as AlipayMerchant).users[0].savedCards.add(cards.find { it.type == 'credit' && !(it.scheme in ['bajajfn', 'amex']) })
        req().get().then()
                .spec(successSchema)
    }

    @Override
    @Merchant(edit = true, value = {  it.acquirings.any { it.payMode == 'dc' && !(it.bank in ['bajajfn', 'amex']) } && it.preferences.saveCard.enabled == true })
    @Test
    void "test able to fetch saved card when debit card is saved on custId and mId"() {
        (m() as AlipayMerchant).users[0].savedCards.clear()
        assert (m() as AlipayMerchant).users[0].savedCards.add(cards.find { it.type == 'debit' && !(it.scheme in ['bajajfn', 'amex']) })
        req().get().then()
                .spec(successSchema)
    }
    ...
    void 'test unable to fetch saved card(s) when checkSum equals random value'() {
        (m() as AlipayMerchant).users[0].savedCards.clear()
        if (!(m() as AlipayMerchant).users[0].savedCards.addAll(cards.find())) throw new SkipException('unable to save card')
        req(m().users[0].id, m().id, UUID.randomUUID().toString()).get().then()
                .spec(errorSchema)
    }
