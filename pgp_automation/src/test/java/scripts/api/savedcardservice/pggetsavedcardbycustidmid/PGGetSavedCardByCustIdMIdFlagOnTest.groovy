...
class PGGetSavedCardByCustIdMIdFlagOnTest extends PGGetSavedCardByCustIdMIdTest {
...
//    @Test(enabled = false)
    ...
    void "test unable to fetch saved card(s) when card is saved on custId & mId given corresponding acquiring is not configured on merchant"() {
        (m() as AlipayMerchant).users[0].savedCards.clear()
        if (!(m() as AlipayMerchant).users[0].savedCards.add(cards.find { it.scheme == 'amex' })) throw new SkipException('unable to save card')
        req().get().then()
                .spec(successSchema)
    }
    ...
    void 'test unable to fetch saved card(s) when mId equals random value'() {
        (m() as AlipayMerchant).users[0].savedCards.clear()
        if (!(m() as AlipayMerchant).users[0].savedCards.add(cards.find())) throw new SkipException('unable to save card')
