...
@Owner('Deepak')
class DeletePGSavedCardByUserIdCardId extends TestSetUp {

    def primary

    final def req = { userId, cardId ->
        given(
                new RequestSpecBuilder()
                        ...
                        .setBaseUri(PGP_HOST)
                        .setBasePath("/savedcardservice/savedCardService/v1/delete/pg/savedcard/userId/cardId/${userId}/${cardId}")
                        .build()
        )
    }
    ...
    void testSuccess() {
        ...
        req(user().id, user().savedCards.find().id).delete().then()
                .statusCode(200)
