...
@Owner('Deepak')
class GetSavedCardByCardIdMIdCustIdUserId extends TestSetUp {

    def primary

    final def req = { cardId, mId, custId, userId ->
        given(
                new RequestSpecBuilder()
                        ...
                        .setBaseUri(PGP_HOST)
                        .setBasePath("/savedcardservice/savedCardService/v1/get/savecard/cardId/mId/custId/userId/${cardId}/${mId}/${custId}/${userId}")
                        .build()
        )
    }
    ...
    void testSuccess() {
        ...
        req(user().savedCards.find().id, m().id, null, user().id).get().then()
                .statusCode(200)
