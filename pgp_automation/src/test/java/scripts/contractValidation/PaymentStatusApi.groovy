...
@Owner("Deepak")
class PaymentStatusApi extends TestSetUp implements ContractTest {
    ...
    final def req = {
        given(
                new RequestSpecBuilder()
                        ...
                        .setBaseUri(PGP_HOST)
                        .setBasePath('/merchant-status/api/v1/getPaymentStatus')
                        .build()
        )
    }
    ...
    @BeforeClass
    void setUp() {
        DriverManager.getDriver().get('https://developer.paytm.com/docs/payment-status-api/')
