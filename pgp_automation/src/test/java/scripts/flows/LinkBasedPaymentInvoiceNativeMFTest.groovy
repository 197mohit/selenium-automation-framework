...
class LinkBasedPaymentInvoiceNativeMFTest extends TestSetUp implements LinkPaymentsFlowTest {
  ...
  /*  private static final String RESP_CODE = "RESPCODE"
    ...
    void "test order is successful when payment is done by nb"() {
        ...
        String txnToken = InitTxn.LinkBasedPaymentInvoiceNativeMF(m().id, m().key, orderId, UUID.randomUUID().toString(), CORRECT_ACCOUNT_NO).execute().jsonPath().getString("body.txnToken")
    ...
    void "test order is successful when wrong account no is passed and validateAccountNumber = true and allowUnverifiedAccount = true"() {
        def orderId = UUID.randomUUID().toString()
        String txnToken = InitTxn.LinkBasedPaymentInvoiceNativeMF(m().id, m().key, orderId, UUID.randomUUID().toString(), WRONG_ACCOUNT_NO, "true", "true").execute().jsonPath().getString("body.txnToken")
        DriverManager.getDriver().get(
                LocalConfig.PGP_HOST + Constants.NativeAPIResourcePath.SHOW_LINK_PAYMENT_PAGE
                        .replace(MID_PLACEHOLDER, m().id)
                        .replace(ORDER_ID_PLACEHOLDER, orderId)
    ...
    void "test order is unsuccessful when wrong account no is passed and validateAccountNumber = true and allowUnverifiedAccount = false"() {
        def orderId = UUID.randomUUID().toString()
        String txnToken = InitTxn.LinkBasedPaymentInvoiceNativeMF(m().id, m().key, orderId, UUID.randomUUID().toString(), WRONG_ACCOUNT_NO, "true", "false").execute().jsonPath().getString("body.txnToken")
        DriverManager.getDriver().get(
                LocalConfig.PGP_HOST + Constants.NativeAPIResourcePath.SHOW_LINK_PAYMENT_PAGE
                        .replace(MID_PLACEHOLDER, m().id)
                        .replace(ORDER_ID_PLACEHOLDER, orderId)
