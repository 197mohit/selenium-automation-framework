...
Install commit-message and branch-naming hooks:

```bash
bash scripts/install-hooks.sh
```

- With **`pre-commit`** installed: uses `.pre-commit-config.yaml` (YAML checks, secret detection, commit-msg, pre-push).
- Without **`pre-commit`**: copies standalone hooks from `.githooks/` into `.git/hooks/`.
- Legacy entry point: `./setup_precommit_hook.sh` (calls `scripts/install-hooks.sh`).

Allowed integration branches for push: `master`, `main`, `release_qa`, `dev_integration`, `dev_integration_*`. Ticket branches: `PGP-<id>`, `PGQA-<id>`.

**CI:** Jenkins runs `scripts/validate-branch-commits.sh origin/release_qa` on every build.
