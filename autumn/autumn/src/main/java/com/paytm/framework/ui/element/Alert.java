...
public class Alert extends UIElement {
    ...
    @TestStep
    public void accept() {
        this.report.info("Accept alert [" + getElementName() + " on [" + getPageName() + "]");
        ...
        try {
            ...
            if (e.getMessage().contains("unknown error: Element is not clickable at point")) {
                this.report.info("Retrying due to the error: Element is not clickable at point(X, Y)");
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e1) {
                    this.report.info("sleeping");
                }
                button.click();
            } else {
                throw e;
    ...
    @TestStep
    public void close() {
        this.report.info("Close alert [" + getElementName() + " on [" + getPageName() + "]");
        WebElement cross = getWrappedElement().findElement(By.cssSelector(".ui-icon.ui-icon-closethick"));
        try {
            cross.click();
        } catch (WebDriverException e) {
            if (e.getMessage().contains("unknown error: Element is not clickable at point")) {
                this.report.info("Retrying due to the error: Element is not clickable at point(X, Y)<br>");
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e1) {
                    this.report.info("sleeping");
